...
hooksecurefunc(GameTooltip, "SetOwner", function(self, owner, anchor)
    if (owner and (owner.unit or (owner.GetAttribute and owner:GetAttribute("unit")))) then
        unitFrameOwner = owner
        -- 检测单位类型，使用 pcall 确保安全
        local unit = owner.unit or owner:GetAttribute("unit")
        local isPlayer = false
        if (unit) then
            local ok, result = pcall(UnitIsPlayer, unit)
            isPlayer = ok and result
        end
        local unitExists = unit and UnitExists(unit)
        if (unitExists) then
            isUnitFramePlayer = isPlayer
            pendingUnitReposition = true
            -- 对于 cursor，立即设置位置（它需要跟随鼠标，必须在 SetOwner 时设置）
            -- cursorRight 在 tooltip:anchor 中处理，避免在 SetOwner hook 中再次调用 SetOwner
            local anchorToUse = isPlayer and GetPlayerAnchor() or GetNpcAnchor()
            if (anchorToUse and anchorToUse.position == "cursor") then
                SetTooltipPositionImmediate(self, owner, anchorToUse, true)
            end
        else
